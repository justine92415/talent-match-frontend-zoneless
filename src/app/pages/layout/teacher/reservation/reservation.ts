...
@Component({
  ...
  imports: [CommonModule, Button, InputSelect, InputText, ReactiveFormsModule, Skeleton],
...
export default class Reservation {
  ...
  teacherCoursesResource = rxResource({
    stream: () => this.teacherService.getApiTeachersMyCourses().pipe(
  ...
  reservationsResource = rxResource({
    ...
    stream: ({ params }) => {
      ...
      return this.reservationService.getApiReservationsCourseReservations(apiParams);
    }
  });
  ...
  onPageChange(page: number) {
    const totalPages = this.reservationsResource.value()?.data?.pagination?.total_pages || 1;
  ...
  onConfirmReservation(reservationId: number) {
    this.dialogService.openConfirm({
      ...
      if (result.confirmed) {
        this.reservationService.postApiReservationsIdConfirm(reservationId).subscribe({
          next: () => {
  ...
  onRejectReservation(reservationId: number) {
    this.dialogService.openConfirm({
      ...
      if (result.confirmed) {
        this.reservationService.postApiReservationsIdReject(reservationId, {
          reason: '教師拒絕預約'
        }).subscribe({
          next: () => {
            this.reservationsResource.reload();
  ...
  // 獲取狀態顯示文字
  getStatusText(teacherStatus: string | undefined, studentStatus: string | undefined): string {
  ...
  // 獲取狀態樣式
  getStatusClass(teacherStatus: string | undefined, studentStatus: string | undefined): string {
    if (teacherStatus === 'pending' && studentStatus === 'reserved') {
